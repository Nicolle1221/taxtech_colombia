...
def test_procesar_xlsx_end_to_end(monkeypatch):
    ...
    ws = wb.active
    ws.title = "Exogena"
    ws.append(["CONTRIBUYENTE", "MARTHA CECILIA HERNANDEZ", "NIT", "10203040-5"])
    ws.append(["Formato", "NIT Informante", "Concepto", "Valor Ingreso", "Valor Retencion"])
    ws.append(["1001", "890903938", 5001, 84_500_000, 3_200_000])
    buffer = io.BytesIO()
    wb.save(buffer)
    ...
    assert respuesta.status_code == 200
    assert "MARTHA CECILIA HERNANDEZ" in texto_recibido["valor"]
    assert "84500000" in texto_recibido["valor"]
    assert respuesta.json()["ingresos_brutos_laborales"] == 84_500_000.0
